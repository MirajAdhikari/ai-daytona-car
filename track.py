...
class Track:
    ...
    def __init__(self):
        ...
        self._build_mask()
        self._build_shapes()
        # finish line bounds (matches the twin white stripes drawn below)
        self.finish_x = (260, 300)
        self.finish_y = (110, 178)

    def crossed_finish(self, x, y):
        return (self.finish_x[0] <= x <= self.finish_x[1] and
                self.finish_y[0] <= y <= self.finish_y[1])
    ...
    def _build_mask(self):
        ...
        for px, py in self.centerline:
            cx, cy = int(px), int(py)
            x0 = max(0, cx - r);  x1 = min(SCREEN_WIDTH,  cx + r + 1)
            y0 = max(0, cy - r);  y1 = min(SCREEN_HEIGHT, cy + r + 1)
            if x1 <= x0 or y1 <= y0:
                continue
